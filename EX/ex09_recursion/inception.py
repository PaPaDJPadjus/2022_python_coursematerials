...
def x_sum_recursion(nums: list, x: int):
    """
...
def sum_squares(nested_list):
    ...
    if not nested_list:
        return 0
    if isinstance(nested_list[0], list) is True:
        return sum_squares(nested_list[0]) + sum_squares(nested_list[0 + 1::])
    if len(nested_list) > 1:
        return nested_list[0] ** 2 + sum_squares(nested_list[0 + 1::])
    else:
        return nested_list[0] ** 2


def count_strings(data: list, pos=None, result: dict = None):
    """
    ...
    if len(data) > pos:
        if isinstance(data[pos], list):
            count_strings(data[pos], pos, result)
            count_strings(data[pos + 1::], pos, result)
