...
def sort_tuples_by_length(tuples: list[tuple]) -> list:
    ...
    """
    sorted_tup = sorted(tuples, key=lambda x: -len(x))
    return sorted_tup
...
def sort_people_by_popularity_of_name(people: list[Person]) -> list:
    ...
    :return: Sorted list of people.
    """
    mapped_list = list(map(lambda x: x.name, people))
    sorted_list = sorted(people, key=lambda x: (-mapped_list.count(x.name), x.name))
    return sorted_list
